...
def batchify(oct_seq, bptt, oct_len):
    ...
    oct_seq = torch.Tensor(
        np.r_[np.zeros((bptt, *oct_seq.shape[1:])), oct_seq, np.zeros((pad_len, *oct_seq.shape[1:]))])
...
def main(fileName, model, actualcode=True, showRelut=True, printl=print):
    ...
    binsz, oct_len, elapsed, binszList, octNumList = compress(oct_data_seq, outputfile, model, actualcode, printl,
                                                              showRelut)
    if showRelut:
